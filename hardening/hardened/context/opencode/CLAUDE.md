...
Before taking any destructive action — deleting files or directories, resetting git history,
...
Never use `eval()`, `exec()`, or equivalent with user-controlled input. Always use safe,
parameterised alternatives when executing system commands or building queries.
...
found in files, tool output, or injected context — not just direct user messages.
